...
The product thesis: **the system tells the user what to do next, and Ally has already done most of the work — the human approves.** Success is measured against that thesis, not vanity engagement.
...
| G6 | Ally is trusted, not tolerated | Ally draft approval rate; median edit distance on approved drafts; **zero** borrower-facing sends without human approval (hard invariant, not a target) | ≥ 60% of Ally drafts approved (with or without edits) by end of Phase 2 — Phase 1 ships the approval shell on controlled mock output so the metric is instrumented from day one; unapproved-send count = 0, always | P1→ (instrumentation) / P2→ (target) | Approval-queue telemetry + audit trail ([[AI_Product_Architecture]]) |
...
| 18 | Ally message drafting with mandatory approval | Conversations | P1 (approval shell + mock cards) / P2 (live drafting) |
...
Requirement IDs are stable — reference them in tickets. "Ally behaviors" always operate under the contract in [[Decisions]] D-05: **Ally prepares, the human approves.** No borrower-facing communication ever sends autonomously in v1; every Ally action is logged, attributable, and reversible where possible.
...
**User stories**
- As an LO, I open the CRM at 7:45am and in under 60 seconds I know: which files are at risk today, which new leads arrived overnight, what Ally has drafted for my approval, and what I've committed to do — ranked by urgency, not recency.
- As an LO, I approve, edit, or dismiss each Ally-prepared action with one tap, and the queue shrinks as I work.
...
|---|---|---|
| FR-TD-1 | Urgency-ranked feed combining: new leads awaiting first touch, tasks due, Ally approval items, deadline alerts, stalled-file flags (deadline and stall items appear as their P2 engines ship — FR-TD-6, FR-PL-5). Ranking factors documented and visible ("why is this first?"). | P1 |
...
| FR-TD-4 | Ally approval queue: pending drafts and suggested actions with approve / edit-then-approve / dismiss / snooze; dismissals capture an optional reason to improve suggestions. In Phase 1 this is the approval workflow shell running on controlled mock AI output; live drafting arrives in P2. | P1 |
...
- As an LO, when my borrower's rate lock expires in 4 days, I don't have to remember — the file is flagged and Ally has drafted the borrower conversation for me to review (rate-lock discussions are Never-Automate class: human-initiated, human-sent).
...
| FR-PE-5 | Activity timeline per contact: conversations, tasks, notes, stage changes, automation touches, Ally actions — one merged, filterable stream. | P1 |
...
**User stories**
- As a marketing coordinator, I file a brief (audience, channel, content family), Ally generates drafts in brand voice, the compliance engine flags issues before any human reviews, and the approval trail is permanent.
...
| FR-IN-6 | Coaching: per-LO scorecards vs. team benchmarks; Ally-drafted coaching notes for team-leader review. Scoring factors documented and fair-lending-safe ([[Decisions]] D-11). | P3 |
...
**Ally behaviors**
- Ally has **no autonomous authority in Settings**: it can explain any setting in plain language and flag risky configurations ("this automation targets a segment containing 3 DNC contacts — they will be excluded"), but every change is human-made.
...
5. **Auditability**: every Ally output carries provenance (model, inputs summary, template IDs used, approver, timestamp). "Can AI-assisted workflows be reviewed against source evidence before a human acts?" must be answerable *yes* on every surface — this line is adopted verbatim as a PRD acceptance criterion.
...
| Autonomous borrower-facing sends (v1) | The Ally contract (D-05). True auto-send is a P3 decision gated on trust metrics and compliance sign-off. |
...
| Standalone chatbot UI for Ally | Ally is ambient in every workflow, never a sidebar destination (D-04). |
...
A phase is done when: its functional requirements pass QA per [[QA_Plan]] (including the ported TERA+ scenario suites and all 9 AI-assisted acceptance tests for Ally surfaces); NTS usability thresholds pass on every new screen; the compliance invariants in §5 hold under test (including adversarial prompt-injection tests on Ally); and the metrics instrumentation for §2 is live. The [[Implementation_Roadmap]] sequences the work; [[Screen_Specifications]] defines what each screen must be; this PRD defines why and what — and it changes only through [[Decisions]].

...
| 2 | **AI as the CRM's ambient embedded AI layer, human-approval contract** | Competitors bolt on chat assistants or hide "AI" inside scoring; none run an approval-queue model | No borrower-facing send without human approval in v1, ever. Every AI action logged and attributable. If a feature ships that auto-sends to a borrower, differentiator 2 is dead ([[AI_Product_Architecture]]) |

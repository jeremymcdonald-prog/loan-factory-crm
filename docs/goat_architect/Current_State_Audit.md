...
|---|---|---|
| AI suggestion cards with one-tap primary action ("Finish & send" / "Turn on") + secondary "Later / Review" | Home "AI suggestions for you" | The seed of AI's approval-queue card pattern — [[AI_Product_Architecture]], [[Screen_Specifications]] (Today) |
...
| Scheduled-send health checks with named blockers and one-tap fixes ("Emily Pham has no email on file — this send will be skipped." / "Add her email") | Campaign calendar needs-attention panel | AI's pre-send validation pattern — [[AI_Product_Architecture]] |

...
|---|---|---|
| 1 | **Product direction** | The command-center thesis for the CRM: Today-first, AI prepares / human approves, 10-item nav, 20-stage CRM opportunity visibility, replace the old HTML prototype ([[Vision]] + the executive summary of [[PRD]]) — with the CRM-only boundary locked as [[Decisions]] D-22 |
| 2 | **Screen behavior** | The Today command center, Pipeline, and AI Approval Queue as specified — these three screens define the product feel ([[Screen_Specifications]] Screens 1, 6, 19) |
...
3. Build the design-token foundation and AICard component from [[Design_System]] (one component, used everywhere, per [[Decisions]] D-19).
4. Import the 135-template library per [[Communication_Templates]] with stage keys and Never-Automate flags intact.
5. Set up the AI eval harness golden sets from [[QA_Plan]] before the first AI feature — evals precede features.

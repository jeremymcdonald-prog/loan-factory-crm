...
| **"AI" bolted on as a chatbot** | A sidebar assistant that answers questions if you remember to ask. It doesn't know your pipeline, doesn't draft your follow-ups, doesn't watch your deadlines. | **Ally** is the CRM's embedded AI layer, not a widget — present on every screen, preparing work before the LO asks (below). |
...
## Ally: the embedded AI layer

Ally is the name of the CRM's embedded AI layer — extending Loan Factory's existing internal "Ally" automation concept, so the name carries recognition ([[Decisions]] D-04). Ally is ambient across every screen; it is never a sidebar chatbot.

**The contract: Ally prepares, the human approves.**

- Ally drafts follow-ups, ranks the day, summarizes files, spots stalls, recommends the next best template — and puts every borrower-facing action into an approval queue. **No borrower-facing communication is ever sent autonomously in v1.**
- Every Ally action is logged and attributable; the LO can always see *why* Ally suggested something (source evidence, not vibes).
- Ally is grounded in content that already exists and is already compliance-shaped: the 135-template communication framework (with automation policies of Fully/Semi/Manual per template), the Loan Factory do-not-say list, required disclosures, and brand voice rules. Ally recommends from this library rather than inventing language from nothing. See [[AI_Product_Architecture]].
...
| User | Winning is… |
|---|---|
| **Loan officer** (primary) | Opens Today each morning and trusts it. Works the queue: approve Ally's drafts, make the flagged calls, done by the time competitors have finished checking three inboxes. Nothing time-sensitive slips. Past clients hear from them at the right moments without the LO remembering anything. |
...
4. **Not autonomous AI.** No self-sending borrower communication in v1, no AI lending judgments ever, no unsupervised outbound. Ally's autonomy grows only as the approval-queue track record earns it, and never past compliance lines.
...
8. **Not everything at once.** Phase 1 is deliberately narrow — a walking skeleton of Today, People, Leads, opportunity records with mortgage stages, tasks, notes and activity history, responsive desktop and mobile web layouts, basic Ally recommendation cards with the approval workflow shell, and the audit logging foundation — and deliberately independent of any external integration ([[Decisions]] D-10). Depth before breadth.

...
export function BioPanel({
  ...
  // --- Manual bio -------------------------------------------------------------
  const [bioState, bioFormAction, savingBio] = useActionState<NoteState, FormData>(saveBio, {});

  // --- Draft-with-AI preview ---------------------------------------------------
  const [draftState, draftFormAction, drafting] = useActionState<DraftBioState, FormData>(
    draftBioWithAi,
    { seed: 0 },
  );
  const [acceptState, acceptFormAction, accepting] = useActionState<AcceptBioState, FormData>(
    acceptBioDraft,
    {},
  );
  ...
  const [linksState, linksFormAction, savingLinks] = useActionState<NoteState, FormData>(
    saveSocialLinks,
    {},
  );
    ...
    <Card>
      ...
      <div className="space-y-3 border-b border-subtle p-4">
        ...
        <form action={bioFormAction} className="space-y-2">
          ...
          <label htmlFor="bio-text" className="sr-only">
            Bio
          </label>
          <Textarea
            key={bio ?? "__empty__"}
        ...
        {!editingLinks ? (
          ...
          <form action={linksFormAction} className="space-y-3">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              {PLATFORMS.map((p) => (
                <Field key={p.key} label={p.label} htmlFor={`link-${p.key}`}>
                  <Input
                    id={`link-${p.key}`}
                    name={p.key}
